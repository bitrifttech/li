...
mod tests {
    ...
    fn test_extract_command() {
        ...
        assert_eq!(
            CommandValidator::extract_command("docker run nginx"),
            Some("docker".to_string())
        );
        assert_eq!(
            CommandValidator::extract_command("/usr/bin/find . -name '*.rs'"),
            Some("/usr/bin/find".to_string())
        );
        assert_eq!(
            CommandValidator::extract_command("echo 'hello world'"),
            Some("echo".to_string())
        );
    ...
    async fn test_check_single_command() {
        ...
        assert!(
            !validator
                .check_single_command("fakecommand123")
                .await
                .unwrap()
        );
    ...
    async fn test_validate_plan() {
        ...
        let plan = planner::Plan {
            ...
            execute_commands: vec!["git status".to_string(), "anotherfakecmd".to_string()],
            confidence: 0.8,
        ...
        assert!(
            result
                .missing_commands
                .iter()
                .any(|cmd| cmd.command == "fakecommand123")
        );
        assert!(
            result
                .missing_commands
                .iter()
                .any(|cmd| cmd.command == "anotherfakecmd")
        );
    ...
    async fn test_cache_operations() {
        ...
        validator
            .command_exists("definitely_not_real_cmd_987654321")
            .await; // should not exist
    ...
    fn test_edge_case_commands() {
        ...
        assert_eq!(
            CommandValidator::extract_command("command-with-dashes"),
            Some("command-with-dashes".to_string())
        );
        assert_eq!(
            CommandValidator::extract_command("command_with_underscores"),
            Some("command_with_underscores".to_string())
        );
        assert_eq!(
            CommandValidator::extract_command("command.with.dots"),
            Some("command.with.dots".to_string())
        );

        // Test commands with paths
        assert_eq!(
            CommandValidator::extract_command("./local-script"),
            Some("local-script".to_string())
        );
        assert_eq!(
            CommandValidator::extract_command("../parent-script"),
            Some("../parent-script".to_string())
        );
        assert_eq!(
            CommandValidator::extract_command("/absolute/path/command"),
            Some("/absolute/path/command".to_string())
        );

        // Test quoted commands
        assert_eq!(
            CommandValidator::extract_command("\"quoted command\""),
            Some("\"quoted".to_string())
        );
        assert_eq!(
            CommandValidator::extract_command("'single-quoted'"),
